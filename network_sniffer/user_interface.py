...
class NetworkSniffer:
    def __init__(self):
        ...
        self.running = True
        self.users_code = "def network_sniffer(packets): \n" + self.tab
        self.run_button = self.render_run_button()
        self.packets = get_packets(10)
        self.packet_ids = self.__get_packet_ids()

    def initialize_screen(self):
        screen = pygame.display.set_mode((self.screen_width, self.screen_height))
        pygame.display.set_caption('Network Sniffer')
        return screen
    ...
    def get_challenge_instructions(self):
        instruction_text = [
            "Instructions:",
            "1. Our network traffic has been compromised and we need you to help us \n retrieve all compromised packets by packet id.",
            "2. The packet class looks like this:\n"
            "{\n"
                f"{self.tab}id: SOME_ID,\n"
                f"{self.tab}Raw:" + " {\n"
                    f"{self.tab}{self.tab}user_id: userpassword\n"
                f"{self.tab}" +"}\n"
            "}\n"
            "3. The packet class only has 2 methods:\n"
                               f"{self.tab}.has_layer() ->" + "(Raw == {})\n"
                                f"{self.tab}.load() --> Raw_object()\n"
    ...
    def validate_user_code(self, user_input):
        try:
            ...
            return self.packet_ids == local_vars['network_sniffer'](self.packets)
        except Exception as e:
            return False

    def __get_packet_ids(self):
        return [packet.id for packet in self.packets]
